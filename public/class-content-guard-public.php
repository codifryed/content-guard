...
class Content_Guard_Public {
	...
	public function enqueue_scripts() {
		...
		$this->run_criteria_for( 'js' );

	}

	private function run_criteria_for( $file_type ) {
		if ( ! empty( $this->options['enable_protection'] ) ) {
			if ( empty( $this->options['disable_for_admins'] ) || ! is_super_admin() ) {
				if ( ! empty( $this->options['protect_all_content'] ) || $this->page_is_protected() ) {
					if ( $file_type == 'js' ) {
						$this->protection_js();
